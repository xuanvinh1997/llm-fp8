...
SparseGPT is a one-shot pruning method, meaning it induces sparsity in a pre-trained model without requiring any subsequent fine-tuning. It is designed to efficiently find an optimal sparse representation of the weights that minimizes the impact on the model's output.
...
Instead of using a fixed, pre-determined mask, SparseGPT can adaptively select which weights to prune. This is done by evaluating the error that would be introduced by pruning each weight and then removing those that cause the least error. The criterion, derived from the Optimal Brain Surgeon framework, is:
\begin{equation}
...
Wanda (short for Weights AND Activations) is another one-shot pruning method that offers a much simpler and faster alternative to SparseGPT.
