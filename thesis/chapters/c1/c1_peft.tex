...
The Low-Rank Adaptation (LoRA) technique is motivated by the hypothesis that the change in weights during model adaptation has a low "intrinsic rank." This suggests that the weight updates can be effectively represented in a much lower-dimensional subspace.
...
QLoRA further reduces the memory footprint of fine-tuning by combining LoRA with quantization. It introduces three key innovations that enable the fine-tuning of extremely large models on a single GPU.
