...
We selected two models from the Llama 3 family \cite{meta2024llama3.1, meta2024llama3.2} for our experiments: Llama-3.2-3B and Llama-3.1-8B. These models provide a representative basis for evaluating our methodology on current state-of-the-art architectures. For fine-tuning, we used a curated 100,000-sample subset of the OpenMathInstruct-2 dataset \cite{toshniwal2024openmath2}, focusing on high-quality mathematical reasoning tasks.



\section{Training Configuration}
The models were trained for three epochs using the AdamW optimizer \cite{loshchilov2017decoupled} with a learning rate of 1e-5. To manage memory constraints while maintaining a large effective batch size, we used a per-device batch size of 6 and a gradient accumulation of 4 steps. The key training parameters are summarized in Table \ref{tab:fp8_config}.
